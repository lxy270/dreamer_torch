...
class Dreamer(nn.Module):
    def __init__(self, obs_space, act_space, config, logger, dataset, discrete_action=-1):
        ...
        # Previous per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/tdmpc2/tdmpc2/walker_run_data/
        # walker_run_state_random_ar1.npz.
        # These follow env_humanoid.py: concatenate [qpos, qvel], flatten the
        # trajectory/time axes, then compute per-feature mean and std (+ 1e-8).
        """
        self.obs_mean = [
            -1.0670396089553833, 0.04794686660170555,
            -0.20851969718933105, 0.5437167286872864,
            -1.2723581790924072, -0.20691923797130585,
            0.5500205159187317, -1.2782247066497803,
            -0.20559655129909515, -0.038555022329092026,
            0.012019999325275421, -0.039094530045986176,
            -0.10160992294549942, 0.11569198966026306,
            -0.06329278647899628, -0.09798011928796768,
            0.11506058275699615, -0.0641215592622757,
        ]
        self.obs_std = [
            0.16772985458374023, 0.4124104678630829,
            1.8604319095611572, 0.6610278487205505,
            1.0902138948440552, 0.6113889813423157,
            0.6592875123023987, 1.090895175933838,
            0.6112408638000488, 0.22361259162425995,
            0.18416057527065277, 0.9557487964630127,
            2.1356570720672607, 2.136545181274414,
            3.026123523712158, 2.132667064666748,
            2.1231908798217773, 3.023453712463379,
        ]
        """

        # Previous per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/tdmpc2/tdmpc2/walker_run_data/
        # walker_run_state_intermix_ar2.npz.
        '''
        self.obs_mean = [
            -0.5482397079467773, 1.379584789276123,
            -0.055852435529232025, 0.6005906462669373,
            -1.537037968635559, 0.18356652557849884,
            0.5835658311843872, -1.53899347782135,
            0.20593808591365814, 0.2817460298538208,
            1.7190643548965454, 0.07450520247220993,
            -0.08361120522022247, -0.005982283037155867,
            0.13893552124500275, -0.07918386906385422,
            0.0027673847507685423, 0.1314377784729004,
        ]
        self.obs_std = [
            0.35480841994285583, 2.065070867538452,
            1.5805933475494385, 0.7861607074737549,
            0.8000829219818115, 0.6464025974273682,
            0.7897540926933289, 0.7962237000465393,
            0.6272996068000793, 0.9830593466758728,
            1.9325913190841675, 4.510165214538574,
            9.743282318115234, 8.627978324890137,
            9.997057914733887, 9.897566795349121,
            8.374154090881348, 9.925383567810059,
        ]
        '''

        # Previous per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/tdmpc2/tdmpc2/hopper_hop_data/
        # hopper_hop_state_random_ar1.npz.
        # These follow env_humanoid.py: concatenate [qpos, qvel], flatten the
        # trajectory/time axes, then compute per-feature mean and std (+ 1e-8).
        """
        self.obs_mean = [
            -0.03848956525325775, -0.9356473088264465,
            -0.11322855949401855, -0.02481042966246605,
            -1.427157998085022, 1.790966272354126,
            0.009463357739150524, -0.005368844605982304,
            -0.001557898591272533, -0.0072642224840819836,
            0.0027470416389405727, -0.003677806118503213,
            0.029085896909236908, 0.011214211583137512,
        ]
        self.obs_std = [
            0.24192015826702118, 0.09811721742153168,
            2.0983874797821045, 0.3126496374607086,
            1.0012555122375488, 0.9746636152267456,
            0.6032922863960266, 0.0935819074511528,
            0.08999604731798172, 0.6993047595024109,
            1.2772225141525269, 1.5476913452148438,
            1.003652811050415, 0.9394813179969788,
        ]
        """

        # Previous per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/tdmpc2/tdmpc2/hopper_hop_data/
        # hopper_hop_state_intermix_ar4.npz.
        """
        self.obs_mean = [
            1.6503044366836548, -0.5258080959320068,
            0.7673969268798828, -0.09531382471323013,
            -1.333831787109375, 1.8781108856201172,
            0.3041627109050751, 0.762239933013916,
            0.07820799946784973, 0.22683122754096985,
            -0.009495998732745647, 0.02091091498732567,
            0.04108000174164772, 0.07364841550588608,
        ]
        self.obs_std = [
            1.5653271675109863, 0.26937156915664673,
            1.3040953874588013, 0.3823411464691162,
            0.8882853388786316, 0.6530938148498535,
            0.553818941116333, 0.7304998636245728,
            1.0553332567214966, 5.057706356048584,
            4.359743595123291, 7.1510748863220215,
            3.397578477859497, 3.3268320560455322,
        ]
        """

        # Previous humanoid intermix per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/data/0522resample/
        # humanoid_walk_state_intermix_ar10.npz.
        # These follow env_humanoid.py: concatenate [qpos, qvel], flatten the
        # trajectory/time axes, then compute per-feature mean and std (+ 1e-8).
        '''
        self.obs_mean = [
            -0.19457270205020905, -0.9422961473464966, 0.9777765274047852,
            0.1891564428806305, 0.07030797004699707, 0.0377877913415432,
            0.3092705309391022, -0.5104855895042419, -0.015810983255505562,
            -0.14020781219005585, -0.16357623040676117,
            -0.8156945705413818, -0.39586615562438965,
            -0.5186899304389954, 0.2534482181072235, -0.366418719291687,
            -0.08969569951295853, 0.11121774464845657,
            -0.9519643783569336, -1.6391912698745728,
            -0.03931864723563194, 0.014908124692738056,
            0.5369387269020081, -0.08709541708230972,
            -1.137536644935608, -0.5053752660751343, 1.0297350883483887,
            0.1114945039153099, -0.19505229592323303,
            -0.45466792583465576, 0.20201116800308228,
            -0.10459787398576736, 0.08107423037290573,
            0.06849852949380875, -0.10636644810438156,
            0.01157089788466692, -0.01171199232339859,
            -0.018789878115057945, -0.15982231497764587,
            -0.028700893744826317, 0.18978777527809143,
            0.08788318932056427, -0.1267818808555603,
            -0.006335417740046978, 0.0526353195309639,
            -0.16756653785705566, -0.04041099175810814,
            0.014825168065726757, -0.024542944505810738,
            0.0705011859536171, 0.09006423503160477,
            -0.2377820611000061, -0.09234573692083359,
            0.1737300157546997, 0.09433268755674362,
        ]
        self.obs_std = [
            2.366842031478882, 2.152158498764038, 0.4114302396774292,
            0.5544569492340088, 0.2126796394586563, 0.2178097814321518,
            0.6776134967803955, 0.3142443597316742, 0.26211440563201904,
            0.2801283895969391, 0.22409473359584808, 0.3314826488494873,
            0.7737608551979065, 0.9406933188438416, 0.6292316913604736,
            0.6662386059761047, 0.19764156639575958, 0.5251611471176147,
            0.7406882047653198, 1.1172919273376465, 0.6860162615776062,
            0.7145039439201355, 0.6085113286972046, 0.6652437448501587,
            0.7715824246406555, 0.6317355036735535, 0.6089127063751221,
            0.9758465886116028, 1.1860284805297852, 1.046673059463501,
            0.9672735333442688, 1.802700400352478, 2.202409505844116,
            3.013221502304077, 2.754340410232544, 3.3880624771118164,
            3.584120988845825, 3.625248908996582, 2.8429887294769287,
            6.100968360900879, 8.41234016418457, 11.451555252075195,
            11.85219955444336, 2.920395851135254, 3.155963897705078,
            8.054719924926758, 13.364651679992676, 14.433948516845703,
            13.598588943481445, 5.480674743652344, 5.295905113220215,
            9.427620887756348, 7.2148051261901855, 5.215030670166016,
            9.752208709716797,
        ]
        '''


        # Active humanoid random per-feature statistics from
        # /scorpio/home/yubei-stu-2/tcond/data/0522resample/
        # humanoid_walk_state_random_ar1.npz.
        
        self.obs_mean = [
            0.023615164682269096, -0.058582503348588943,
            0.22799022495746613, 0.4960794448852539,
            0.38669654726982117, 0.24828560650348663,
            0.27730828523635864, -0.07299534976482391,
            -0.09365042299032211, -0.09090764820575714,
            -0.07353755086660385, -0.09017784148454666,
            -0.04949027672410011, -1.2310022115707397,
            -0.042416445910930634, 0.09132074564695358,
            -0.032916583120822906, 0.01697598211467266,
            -0.07128046452999115, -1.165591835975647,
            -0.040649522095918655, 0.03411073982715607,
            0.18001019954681396, -0.3333471119403839,
            -0.12406089901924133, -0.03189881518483162,
            0.32208049297332764, -0.07837814837694168,
            0.0032758237794041634, -0.008707673288881779,
            -0.27735963463783264, -0.06784120202064514,
            -0.09251926839351654, -0.025135286152362823,
            -0.01627224124968052, 0.06831762939691544,
            -0.02125656045973301, 0.019730370491743088,
            0.02766571007668972, 0.15503281354904175,
            0.03765086084604263, -0.0070958989672362804,
            0.015510368160903454, 0.029521051794290543,
            0.05681391805410385, 0.16661858558654785,
            0.05347476154565811, -0.004984525963664055,
            0.005457617808133364, 0.07075335830450058,
            -0.023767434060573578, 0.0609610453248024,
            -0.034725189208984375, 0.02514748089015484,
            0.0798555240035057,
        ]
        self.obs_std = [
            0.21695059537887573, 0.21814365684986115,
            0.30800309777259827, 0.2655758559703827,
            0.31239113211631775, 0.40990445017814636,
            0.3563305735588074, 0.22454971075057983,
            0.31152546405792236, 0.25119107961654663,
            0.14179742336273193, 0.41509172320365906,
            0.30464616417884827, 0.9432171583175659,
            0.4671477675437927, 0.5094635486602783,
            0.12505285441875458, 0.30580398440361023,
            0.311634361743927, 0.907245397567749,
            0.47074297070503235, 0.5209484100341797,
            0.7772060632705688, 0.5955765843391418,
            0.9090051054954529, 0.7873099446296692,
            0.6708378195762634, 0.8807896375656128,
            0.21783897280693054, 0.21786484122276306,
            0.8638436794281006, 0.8337993621826172,
            1.2813441753387451, 1.75540292263031,
            2.092588186264038, 2.6227118968963623,
            1.550653100013733, 1.8704198598861694,
            2.3037736415863037, 3.7349257469177246,
            5.652199745178223, 8.727121353149414,
            9.043207168579102, 1.6242938041687012,
            2.2587788105010986, 3.6898930072784424,
            6.014341354370117, 8.912249565124512,
            9.257612228393555, 2.8021485805511475,
            3.2092437744140625, 6.32766580581665,
            3.28593111038208, 3.514974594116211,
            6.652413368225098,
        ]
        
        # Inference loaders can provide checkpoint-specific statistics. This
        # takes precedence over the manually selected training constants above.
        config_obs_mean = getattr(config, "obs_mean", None)
        config_obs_std = getattr(config, "obs_std", None)
        if (config_obs_mean is None) != (config_obs_std is None):
            raise ValueError("config.obs_mean and config.obs_std must be provided together")
        if config_obs_mean is not None:
            self.obs_mean = list(config_obs_mean)
            self.obs_std = list(config_obs_std)
        ...
        self._logger = None # SummaryWriter(log_dir=self._config.logdir)
        self.update_best_ckpt = False
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self._wm._model_opt._opt, T_max=60, eta_min=1e-10)
    ...
    def propiro_pred(self, data, condition_steps=1):
        """Predict raw states from raw observations/actions.

        This is the public Dreamer-level inference entry point. It applies the
        same observation normalization used during training without mutating
        ``data``, then converts both world-model outputs back to raw units.
        Actions are intentionally left unchanged because Dreamer was trained
        with raw actions.
        """
        if "targets" not in data:
            raise KeyError("propiro_pred expects data['targets']")

        normalized_data = dict(data)
        normalized_data["targets"] = self._normalize_obs(data["targets"])
        prediction, reconstruction = self._wm.propiro_pred(
            normalized_data, condition_steps=condition_steps
        )
        return (
            self._denormalize_prediction(prediction),
            self._denormalize_prediction(reconstruction),
        )
...
def main(config):
    ...
    if config.nq != 0: # dm_phy
        ...
        obs_space = gym.spaces.Dict({'state': gym.spaces.Box(-np.inf, np.inf, (18,), dtype=np.float32),}) # for tcond
            ...
            action_space = gym.spaces.Box(-np.inf, np.inf, (6,), dtype=np.float32)
            obs_space = gym.spaces.Dict({'state': gym.spaces.Box(-np.inf, np.inf, (18,), dtype=np.float32),}) # for tcond
    elif 'hopper' in config.task:
            config.num_actions = 4
            action_space = gym.spaces.Box(-np.inf, np.inf, (4,), dtype=np.float32)
            obs_space = gym.spaces.Dict({'state': gym.spaces.Box(-np.inf, np.inf, (14,), dtype=np.float32),}) # for tcond
